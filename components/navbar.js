...
function MobileNav ( { open, setOpen } )
{
  return (
    <div className={ `absolute top-0 left-0 h-screen w-screen bg-primary transform ${ open ? "-translate-x-0" : "-translate-x-full" } transition-transform duration-300 ease-in-out filter drop-shadow-md ` }>
      <div className="flex items-center filter drop-shadow-md text-primary bg-base-100 text-5xl h-20 p-5"> {/*Logo Container*/ }
        <Link href="/" className="btn btn-ghost normal-case text-xl">Growl</Link>
      </div>
    ...
    <nav className="flex filter drop-shadow-md bg-primary text-base-100 font-bold p-5 h-20 items-center">
      ...
      <div className="w-9/12 flex justify-end items-center">
        ...
        <div className="hidden md:flex">
          ...
          <input type="text" placeholder="Search" className="input input-ghost w-full max-w-xs border-x-0 border-t-0 border-b-4 border-neutral text-neutral ml-5" />
